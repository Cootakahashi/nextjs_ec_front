...
export async function getStaticProps() {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/api/auth/new-products/`
  );
  const data = await res.json();
  const products = data.results; // results キーから製品を取得
  const Gridproducts = products.filter((product) => product.recommend !== null);
...
function NewProducts({ Gridproducts, image, SideSections, GridBrandSections }) {
  return (
    <div>
      <SearchBar />
      <Header />
      <Top image={image} name={"New Arrival"} />
